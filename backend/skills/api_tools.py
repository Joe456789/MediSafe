...
REQUEST_TIMEOUT = 10  # seconds; these are external gov APIs that can stall or go unresponsive
...
def fetch_pubmed_abstracts(ingredient: str) -> str:
    ...
    try:
        search_res = requests.get(search_url, timeout=REQUEST_TIMEOUT).json()
        ...
        summary_res = requests.get(summary_url, timeout=REQUEST_TIMEOUT).json()
